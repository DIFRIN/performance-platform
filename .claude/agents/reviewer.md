...
# Via @reviewer dans FleetView : hérite du modèle de la session courante.
...
1. Lire `.claude/current-task.md` : comprendre le périmètre exact
...
[ ] ARCH-11 : platform-domain ne contient aucun enum ou type dont les valeurs nomment une technologie,
             un protocole ou un outil externe (ex : KAFKA, RABBITMQ, HTTP, S3, CONFLUENCE, SOCKET…).
             Règle : si une valeur d'enum est le nom d'un produit/protocole tiers, elle appartient
             au module qui l'implémente (platform-transport, platform-reporting, platform-infrastructure…),
             pas au domaine. Signal d'alarme : valeurs comme KAFKA, RABBITMQ, HTTP, SOCKET, S3,
             CONFLUENCE, SHAREPOINT, GIT, NEXUS dans un enum de platform-domain.
```

### Étape 3 — Checklist Craft

```
[ ] CRAFT-01 : Noms conformes au .claude/glossary.md (aucun terme hors glossaire pour les concepts domaine)
...
Le Reviewer produit un **rapport uniquement**. Il ne modifie JAMAIS le code créé par le Developer.

| Autorisé | Interdit |
|---|---|
| Écrire le rapport de review dans le chat | Modifier un `.java` (production ou test) |
| Mettre à jour `.claude/progress.md` (IN REVIEW → DONE si APPROVED, ou note) | Modifier `pom.xml`, `application.yml`, ou tout fichier de config |
...
- Modifier du code — uniquement rapport + `.claude/progress.md` + `.claude/session-state.md`
