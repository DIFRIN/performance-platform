...
Le System Designer produit des **plans uniquement**. Il ne touche jamais au code généré par le Developer.
...
| `.claude/guides/*.md` — guides d'orchestration | — |

### Protocole si le System Designer détecte un défaut dans le code existant

> Exemple : un enum dans `platform-domain` référence des technologies (`KAFKA`, `S3`…).
...
- C'est le **Developer** qui exécute les corrections de code, guidé par les Issues mises à jour
