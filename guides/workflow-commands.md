...
ls -la
# Attendu : CLAUDE.md  agents/  adr/  context/  guides/  issues/  pdr/
#           progress.md  prompts/  session-state.md  specifications/  skills/
...
> **But** : transformer les specs en PDRs et Issues dans `progress.md`.
...
6. specifications/01-scenario-dsl.md
7. specifications/02-execution-engine.md
8. specifications/03-task-framework.md
9. specifications/04-agent-runtime.md
10. specifications/05-transport-layer.md
11. specifications/06-injection-gatling.md
12. specifications/07-assertion-framework.md
13. specifications/08-report-engine.md
14. specifications/09-deployment.md
15. adr/*.md
16. context/interfaces-registry.md
...
- `pdr/PDR-001-*.md` ... `pdr/PDR-NNN-*.md`
- `issues/ISSUE-001-*.md` ... `issues/ISSUE-NNN-*.md`
- Met à jour `progress.md`
- Met à jour `context/interfaces-registry.md`

### 0.3 — Commit après System Designer

```bash
git add pdr/ issues/ progress.md context/interfaces-registry.md
git commit -m "feat: system designer — PDRs and Issues created

$(grep -c 'PDR-' progress.md) PDRs, $(grep -c 'ISSUE-' progress.md) Issues
First actionable: $(grep -m1 'WAITING' issues/*.md 2>/dev/null | head -1 | cut -d/ -f2)"
```
...
grep -A1 "IN PROGRESS\|IN REVIEW" progress.md | grep "ISSUE-"

# Voir la prochaine Issue disponible
grep -B1 "WAITING" progress.md | grep "ISSUE-" | head -5
...
```bash
git add progress.md context/interfaces-registry.md
...
2. issues/ISSUE-XXX.md  — critères de done
3. pdr/PDR-XXX.md       — si points ARCH concernaient des interfaces
...
2. issues/ISSUE-XXX.md              — composants à tester
3. pdr/PDR-XXX.md                   — interfaces exactes
4. progress.md                      — voir quelles Issues sont DONE (éviter doublons)

PROCESSUS :
1. Définis ton plan de test
2. Écris les tests d'intégration Testcontainers
3. Exécute et rapport
4. Mets à jour feature-summaries/README.md
5. Mets à jour session-state.md
```
...
```
Lis session-state.md et progress.md. Dis-moi quelle Issue est active et quelle est la prochaine action.
```

**→ Selon la réponse, copier le prompt correspondant depuis `prompts/`.**
...
| Reprise / état | `claude-dev` | `Lis session-state.md et progress.md, état actuel ?` |
...
# État global
echo "=== PROGRESS ===" && \
grep -E "WAITING|IN PROGRESS|IN REVIEW|✅ DONE|BLOCKED" progress.md | \
