...
> Le contexte général est dans `session-state.md` — pas besoin de relire les specs.
...
Fichiers obligatoires (~400 tokens) :
  session-state.md          ~80 tokens
  CLAUDE.md                 ~300 tokens
  current-task.md           ~150 tokens
...
Fichiers conditionnels (lire UNIQUEMENT si dans session-state.md) :
  ...
  glossary.md               ~200 tokens   — si un terme est ambigu
  context/interfaces-registry.md  ~300 tokens  — toujours (évite les divergences)
...
À NE PAS charger : autres specs, roadmap, adr/, guides/, feature-summaries
```
...
Fichiers conditionnels :
  feature-summaries/README.md (index) ~80 tokens
...
Fichiers obligatoires :
  ...
  adr/*.md                  ~200 tokens (tous, compact)
  architecture.md           ~300 tokens

Fichiers conditionnels :
  spec concernée (section précise) ~150 tokens
  context/interfaces-registry.md  ~300 tokens si interfaces impactées
...
Indiquer dans `current-task.md` et `session-state.md` **quelle section** lire,
...
Si une information est dans `session-state.md` → ne pas aller la chercher dans la spec.
Si une signature est dans `context/interfaces-registry.md` → ne pas aller la chercher dans la spec.
Si une décision est dans `context/decisions-log.md` → ne pas aller la chercher dans les ADRs.
...
```
1. Lire session-state.md               → 30 tokens + décision sur "Files to Load"
...
4. Travailler
5. En fin de session :
   - Mettre à jour session-state.md     → maintient le cache pour la prochaine session
...
❌ Charger guides/ et adr/ à chaque session
❌ Demander un résumé du projet à chaque session au lieu de lire session-state.md
